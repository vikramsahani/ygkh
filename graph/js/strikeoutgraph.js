(function() {
  ...
  if (window.location.toString().indexOf('=') == -1)
    ...
    csvfile = window.location.toString().substr(window.location.toString().indexOf('=') + 1);
    csvfile = decodeURIComponent(csvfile.replace(/\+/g, ' '));
  }
  var countryname = csvfile;
  csvfile += ".csv";
...
queue()
    .defer(d3.csv, "csv/countrygraph/"+csvfile)
    .defer(d3.csv, "csv/code.csv")
    //.defer(d3.csv, "csv/average-by-count.csv")
    .await(ready)
...
function ready(err, strikeouts, teams) {
  ...
  strikeouts.forEach(function(d) {
      ...
      d.hrpg = +d.hrpg;
      d.league = teamByCode[d.indicator].league;
  });
  ...
  var leagues = d3.nest()
    ...
    .rollup(function(values){
      ...
      return {
        ...
        rank: values.sort(function(a, b) { return b.value - a.value}).map(function(d) { return d.year; }).indexOf(2011) + 1,
        lastYear: values.filter(function(d) { return d.year === 2011; })[0].value
      };
    })
    .entries(strikeouts);
  ...
  var avgData = drawLineChart(svg, strikeouts, "value", x, y, width, height, 3, bounds);
  ...
  avgData.forEach(function(d, i) {
    if (lowestvalue > d.values.avg) {
      lowestvalue = d.values.avg;
      lowestYear = d.key;
    }
    if (highestvalue < d.values.avg) {
      highestvalue = d.values.avg;
      highestYear = d.key;
    }
  });
  ...
  //     .attr("transform", "translate(" + x(highestYear) + "," + y(highestvalue) + ")")

  // highestLabel.append("text")
  //     .attr("class", "number")
  //     .attr("dy", "-10px")
  //     .text(format(highestvalue));
  ...
  //     .attr("class", "g-mean-text g-hide-hover")
  //     .attr("transform", "translate(" + x(lowestYear) + "," + y(lowestvalue) + ")")

  // lowestLabel.append("text")
  //     .attr("class", "number")
  //     .attr("dy", "30px")
  //     .text(format(lowestvalue));
  ...
  function drawLegend(container, headline) {
    ...
    teamChooser.on("change", function() { selectTeam(this.value); })
      ...
      .enter().append("optgroup")
        .attr("label", function(d) { return ("").toUpperCase(); })
      .selectAll("option")
  ...
  function drawLineChart(container, data, attributeY, x, y, width, height, r, bounds) {
    ...
    teamsContainer.append("g")
      ...
      .enter().append("path")
        ...
        .on("click", function(d, i) { selectTeam(d.data.indicator); });
    ...
    function selectValue(d) {
      if (d) {
        var offset = averageData[averageData.length - (2011 - d.year) - 1].values.avg < d[attributeY] ? -28 : +32;
        ...
        hoverTeam.attr("y", offset).text(teamByCode[d.indicator].name);
